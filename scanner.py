...
def try_endpoint(args, site, endpoint):
    ...
    write_r = requests.post(url, data="null", headers={
                            'Content-type': 'application/json'})
    data = read_r.json() if read_r.status_code != 401 else None
    print("Site: " + site + " endpoint: " +
...
def scan_sites(args):
    ...
    for site in firebase_sites:
        ...
        if args.endpoints:
            ...
            # If there are endpoints specified, use them!
        ...
        for endpoint in endpoint_list:
            ...
            read_success, write_success, data = try_endpoint(
                args, site, endpoint)
            if data is not None:
                for part in dump_path:
                    ...
                    if read_success or write_success:
                        endpoint_info[endpoint] = {
                            "read": read_success, "write": write_success}
...
def parse_args():
    ...
    parser.add_argument(
        '--sites', help="File containing list of firebase sites to scan [code].firebaseio.com", type=FileType('r'))
    ...
    parser.add_argument('--dirty', help="Should we modify the db to find more writable locations?",
                        action="store_true", default=False)
    parser.add_argument('--out', help="A file to dump info to",
